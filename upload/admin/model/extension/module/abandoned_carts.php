...
class ModelExtensionModuleAbandonedCarts extends Model {
	...
	public function recoverEmail($order_id) {
		...
		$text  = sprintf($this->language->get('failed_cart_greeting'),ucfirst($order_info['firstname']))."\n\n";
		...
		$mail->setSubject($this->language->get(html_entity_decode($this->language->get('subject_prefix')).' '.$order_info['store_name']));
		$mail->setText($text);
		$mail->send();
	...
	public function getOrders($data = array()) {
		...
		$sql .= " WHERE o.date_added >= DATE_SUB(NOW(), INTERVAL ".$this->config->get('module_abandoned_carts_limit')." DAY) && NOW() >= DATE_ADD(o.date_added, INTERVAL 1 HOUR)  && o.firstname !='' && o.lastname !='' && o.order_status_id=0";
	...
	public function getTotalOrders($data = array()) {
		...
		$sql = "SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` o WHERE o.date_added >= DATE_SUB(NOW(), INTERVAL ".$this->config->get('module_abandoned_carts_limit')." DAY) && NOW() >= DATE_ADD(o.date_added, INTERVAL 1 HOUR)  && o.firstname !='' && o.lastname !='' && o.order_status_id=0";
